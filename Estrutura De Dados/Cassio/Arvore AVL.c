...
int altura_NO(struct NO* no){ //NOVO funcao para acessar um campo
    if(no == NULL)
        return -1;
    else
    return no->altura;
...
int fatorBalanceamento_NO(struct NO* no){ //NOVO
    return labs(altura_NO(no->esq) - altura_NO(no->dir));
}

int maior(int x, int y){ //NOVO
    if(x > y)
        return x;
    else
        return y;
...
int estaVazia_ArvAVL(ArvAVL *raiz){ //NOVO
    if(raiz == NULL)
        return 1;
    if(*raiz == NULL)
        return 1;
